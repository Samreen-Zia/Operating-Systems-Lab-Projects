...
* **Task 2: Gross Salary Calculator:** Implementation of conditional logic to calculate HRA (10% or 20%) and total salary based on basic pay input.[cite: 1]
...
Explored the lifecycle of processes and the hierarchy of parent-child relationships using system calls.[cite: 2]

* **Task 1: Sibling Processes:** Using the `fork()` system call to create an architecture where a single parent creates two distinct child processes (P1 and P2).[cite: 2]
...
* **Task 1: Data Appending:** Copying data from a source file (F1) to a destination file (F2) using the `O_APPEND` flag to ensure existing data is preserved.[cite: 3]
